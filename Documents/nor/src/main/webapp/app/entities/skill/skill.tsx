  ...
  return (
    <div>
      <h2 id="skill-heading" data-cy="SkillHeading">
        <Translate contentKey="norApp.skill.home.title">Skills</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="norApp.skill.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="norApp.skill.home.createLabel">Create new Skill</Translate>
          </Link>
        </div>
      </h2>
        ...
        {skillList && skillList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="norApp.skill.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {skillList.map((skill, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{skill.name}</td>
                  <td>
                    {skill.seniorityLevel ? <Link to={`seniority-level/${skill.seniorityLevel.id}`}>{skill.seniorityLevel.name}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="norApp.skill.home.notFound">No Skills found</Translate>
            </div>
          )
        )}
      </div>
